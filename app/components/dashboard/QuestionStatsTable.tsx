                            ...
                            <div className="text-xl font-semibold">
                                {stats && `R ${(
                                    ...
                                    wordRecordingStats
                                        ...
                                        .reduce((sum, stat) => sum + stat.totalRecordings, 0)
                                ).toLocaleString()}`}
                            </div>
                        </div>
                    </div>
                            ...
                            {languageStats.length > 0 || wordRecordingStats.length > 0 ? (
                                <>
                                    {languageStats.map((stat, index) => {
                                        ...
                                        const totalDue = isBenedict ? 'N/A' : `R ${(stat.questionCount + wordRecording.totalRecordings).toLocaleString()}`;
                                    ...
                                    {wordRecordingStats
                                        ...
                                        .map((stat, index) => (
                                            <tr key={`word-${index}`}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {stat.learnerName === 'Benedict Nkosi' ? 'N/A' : `R ${stat.totalRecordings.toLocaleString()}`}
                                                </td>
                                            </tr>
                                        ))
                                    }
